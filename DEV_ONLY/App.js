...
const SPEED_DATA = {
  ...
  dataLabels: {
    format:
      '<div style="text-align:center;width:100px"><span style="font-size:36px;color:black">{y}</span><br/>' +
...
const gaugeConfig = defaultConfig
  ...
  .yAxis({
    ...
    stops: [[0.25, GREEN], [0.5, YELLOW], [0.75, RED]],
    tickAmount: 2,
    title: {
      text: 'Speed',
      y: -145,
...
const generalDrawingConfig = buildConfig()
  ...
  .chart('events.load', function() {
    ...
    // Headers
    renderer
      .label('Web client', 20, 40)
      .css({
        fontWeight: 'bold',
      })
      .add();
    renderer
      .label('Web service / CLI', 220, 40)
      .css({
        fontWeight: 'bold',
      })
      .add();
    renderer
      .label('Command line client', 440, 40)
      .css({
        fontWeight: 'bold',
      })
      .add();
    ...
    // SaaS client label
    renderer
      .label('SaaS client<br/>(browser or<br/>script)', 10, 82)
      .attr({
        fill: colors[0],
        padding: 5,
        r: 5,
    ...
    renderer
      .label('POST options in JSON', 90, 75)
      .css({
        color: colors[3],
        fontSize: '10px',
      })
    ...
    renderer
      .path(['M', 250, 110, 'L', 250, 185, 'L', 245, 180, 'M', 250, 185, 'L', 255, 180])
      .attr({
        stroke: colors[3],
        'stroke-width': 2,
      })
    ...
    renderer
      .label('Rasterized image', 100, 110)
      .css({
        color: colors[3],
        fontSize: '10px',
      })
    ...
    // Browser label
    renderer
      .label('Browser<br/>running<br/>Highcharts', 10, 180)
      .attr({
        fill: colors[0],
        padding: 5,
        r: 5,
    ...
    renderer
      .label('Rasterized image', 100, 215)
      .css({
        color: colors[1],
        fontSize: '10px',
      })
    ...
    renderer
      .label('Rasterized image', 330, 100)
      .css({
        color: colors[2],
        fontSize: '10px',
      })
